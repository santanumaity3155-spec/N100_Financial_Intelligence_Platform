...
class DataValidator:
    ...
    def validate_required_columns(
        self, df: pd.DataFrame, required_columns: List[str], dataset_name: str
    ) -> Dict[str, Any]:
        """
        ...
        if not required_columns:
            result["message"] = (
                f"No required columns specified. Dataset has {len(df_columns)} columns."
            )
            logger.info(
                f"{dataset_name}: No required columns specified, dataset has {len(df_columns)} columns"
            )
            return result
    ...
    def detect_missing_values(
        ...
        if columns_to_check:
            ...
            if not columns_to_check:
                logger.info(
                    f"{dataset_name}: No valid columns to check for missing values"
                )
                return result
        else:
            # Check all columns
    ...
    def detect_duplicates(
        self, df: pd.DataFrame, dataset_name: str, subset: Optional[List[str]] = None
    ) -> Dict[str, Any]:
        """
        ...
        if not subset:
            logger.info(
                f"{dataset_name}: Skipping duplicate check (no valid subset columns)"
            )
        ...
        try:
            ...
            if duplicate_count > 0:
                result["passed"] = False
                result["message"] = (
                    f"Found {duplicate_count} duplicate rows " f"({duplicate_pct:.2f}%)"
                )
                logger.warning(
    ...
    def validate_data_types(
        self, df: pd.DataFrame, expected_types: Dict[str, str], dataset_name: str
    ) -> Dict[str, Any]:
        """
        ...
        for col, expected_type in expected_types.items():
            if col not in df.columns:
                result["type_mismatches"].append(
                    {"column": col, "issue": "Column not found"}
                )
                continue
            ...
            if expected_type.lower() in ["int", "integer"] and "int" in actual_type:
                ...
                expected_type.lower() in ["float", "numeric"] and "float" in actual_type
            ):
                type_match = True
            elif (
                expected_type.lower() in ["str", "string", "text"]
        ...
        if result["type_mismatches"]:
            result["passed"] = False
            result["message"] = (
                f"Type mismatches found: {len(result['type_mismatches'])}"
            )
    ...
    def validate_dataset(
        ...
        # Check duplicates
        dup_check = self.detect_duplicates(df, dataset_name, subset=duplicate_subset)
        ...
        # Check data types
        if expected_types:
            type_check = self.validate_data_types(df, expected_types, dataset_name)
    ...
    def get_validation_summary(self) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "failed": failed,
            "pass_rate": round(
                (passed / total_datasets * 100) if total_datasets > 0 else 0, 2
            ),
            "datasets": {},
        }
...
def validate_dataset(df: pd.DataFrame, dataset_name: str, **kwargs) -> Dict[str, Any]:
    """
