...
class KPIValidator:
    ...
    def validate_kpi_result(
        self, kpi_name: str, value: Any, expected_type: type = float
        ...
        if not isinstance(value, (expected_type, int)):
            return (
                False,
                f"{kpi_name}: Expected {expected_type.__name__}, got {type(value).__name__}",
            )
    ...
    def validate_data_completeness(
        ...
        logger.debug(
            f"Data completeness validation: {report['completeness_score']}% complete"
        )
