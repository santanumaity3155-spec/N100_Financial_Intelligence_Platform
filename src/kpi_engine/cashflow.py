...
class CashFlowCalculator:
    ...
    def calculate_all(self, company_id: str, period: str) -> Dict[str, Any]:
        ...
        results = {
            ...
            "cash_conversion_ratio": self.calculate_cash_conversion_ratio(
                cf_data, pl_data
            ),
        }

        logger.info(
            f"Calculated {len([v for v in results.values() if v is not None])} cash flow KPIs"
        )
        return results
    ...
    def calculate_operating_cash_flow(self, cf_data: pd.DataFrame) -> Optional[float]:
        ...
        try:
            ...
            if cf_data.empty:
                logger.warning(
                    "Operating Cash Flow calculation: Empty dataframe provided"
                )
                return None
            ...
            if ocf is None or pd.isna(ocf):
                logger.warning(
                    "Operating Cash Flow calculation: Missing cash flow data (tried 'cash_from_operating_activity' and 'operating_activity')"
                )
                return None
            ...
            logger.error(
                f"Operating Cash Flow calculation failed - IndexError (empty dataframe): {str(e)}"
            )
            return None
    ...
    def calculate_free_cash_flow(self, cf_data: pd.DataFrame) -> Optional[float]:
        ...
        try:
            ...
            if fcf is not None and not fcf.isna().all():
                ...
                if not pd.isna(fcf):
                    logger.debug(
                        f"Free Cash Flow calculated from direct value: {fcf:.2f}"
                    )
                    return round(fcf, 2)
            ...
            if ocf is None or pd.isna(ocf):
                logger.warning(
                    "Free Cash Flow calculation: Missing operating cash flow data"
                )
                return None
            ...
            logger.error(
                f"Free Cash Flow calculation failed - IndexError (empty dataframe): {str(e)}"
            )
            return None
        except Exception as e:
    ...
    def calculate_cash_conversion_ratio(
        self, cf_data: pd.DataFrame, pl_data: pd.DataFrame
    ) -> Optional[float]:
        """
        ...
        try:
            ...
            if cf_data.empty or pl_data.empty:
                logger.warning(
                    "Cash Conversion Ratio calculation: Empty dataframes provided"
                )
                return None
            ...
            if ocf is None or net_profit is None or pd.isna(ocf) or pd.isna(net_profit):
                logger.warning(
                    "Cash Conversion Ratio calculation: Missing OCF or net profit data"
                )
                return None
            ...
            logger.error(
                f"Cash Conversion Ratio calculation failed - IndexError (empty dataframe): {str(e)}"
            )
            return None
        except Exception as e:
