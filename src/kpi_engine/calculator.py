...
class KPIEngine:
    ...
    def calculate_all_kpis(self, company_id: str, period: str) -> Dict[str, Any]:
        ...
        logger.info(
            f"Calculated {len([v for v in results.values() if v is not None])} total KPIs"
        )
        return results

    def calculate_growth_kpis(
        self, company_id: str, periods: List[str]
    ) -> Dict[str, Any]:
        """
        ...
        """
        logger.info(
            f"Calculating growth KPIs for {company_id} across {len(periods)} periods"
        )
    ...
    def calculate_kpis_for_company(
        self, company_id: str, periods: List[str]
    ) -> List[Dict[str, Any]]:
        """
    ...
    def calculate_kpis_for_all_companies(
        self, company_ids: List[str], period: str
    ) -> List[Dict[str, Any]]:
        """
        ...
        """
        logger.info(
            f"Calculating KPIs for {len(company_ids)} companies, period {period}"
        )
        ...
        logger.info(
            f"Successfully calculated KPIs for {len(results)}/{len(company_ids)} companies"
        )
        return results
    ...
    def validate_kpi_results(self, kpi_batch: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        logger.info(
            f"Validation complete: {validation_results['valid_kpis']}/{validation_results['total_kpis']} KPIs valid"
        )
        return validation_results

    def format_kpi_results(
        self, kpi_batch: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        """
        ...
        formatted_results = [
            self.formatter.format_kpi_results(kpi) for kpi in kpi_batch
        ]
    ...
    def save_kpi_results(
        self, kpi_batch: List[Dict[str, Any]], output_dir: str
    ) -> Dict[str, bool]:
        """
        ...
        results["json"] = self.formatter.save_kpi_results_to_json(
            kpi_batch, str(json_path)
        )

        # Save to CSV
        csv_path = output_path / "kpi_results.csv"
        results["csv"] = self.formatter.save_kpi_results_to_csv(
            kpi_batch, str(csv_path)
        )
...
def calculate_kpis(
    company_id: str, period: str, db_path: Optional[str] = None
) -> Dict[str, Any]:
    """
...
def calculate_kpis_batch(
    company_ids: List[str], period: str, db_path: Optional[str] = None
) -> List[Dict[str, Any]]:
    """
