...
class GrowthCalculator:
    ...
    def calculate_all(self, company_id: str, periods: List[str]) -> Dict[str, Any]:
        ...
        if len(periods) < 2:
            logger.warning(
                f"Insufficient periods for growth calculation: {len(periods)}"
            )
            return {}
        ...
        logger.info(
            f"Calculated {len([v for v in results.values() if v is not None])} growth KPIs"
        )
        return results
    ...
    def calculate_profit_cagr(self, pl_data: pd.DataFrame) -> Optional[float]:
        ...
        try:
            if (
                "net_profit" not in pl_data.columns
                or pl_data["net_profit"].isna().all()
            ):
            ...
            if beginning_value <= 0 or ending_value <= 0:
                logger.warning(
                    "Profit CAGR calculation: Invalid profit values (negative or zero)"
                )
                return None
    ...
    def calculate_margin_expansion(self, pl_data: pd.DataFrame) -> Optional[float]:
        ...
        try:
            if "net_profit" not in pl_data.columns or "sales" not in pl_data.columns:
                logger.warning(
                    "Margin Expansion calculation: Missing net_profit or sales data"
                )
                return None
            ...
            if len(margins) < 2:
                logger.warning(
                    "Margin Expansion calculation: Insufficient valid margin data points"
                )
                return None
            ...
            logger.debug(
                f"Margin Expansion calculated: {expansion:.2f} percentage points"
            )
