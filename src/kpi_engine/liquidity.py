...
class LiquidityCalculator:
    ...
    def calculate_all(self, company_id: str, period: str) -> Dict[str, Any]:
        ...
        if bs_data.empty:
            logger.warning(
                f"Insufficient balance sheet data for {company_id}, period {period}"
            )
            return {}
        ...
        logger.info(
            f"Calculated {len([v for v in results.values() if v is not None])} liquidity KPIs"
        )
        return results
    ...
    def calculate_current_ratio(self, bs_data: pd.DataFrame) -> Optional[float]:
        ...
        try:
            ...
            if (
                ...
                logger.warning(
                    "Current Ratio calculation: Missing current_assets or current_liabilities data"
                )
                return None
    ...
    def calculate_quick_ratio(self, bs_data: pd.DataFrame) -> Optional[float]:
        ...
        try:
            ...
            if (
                ...
                logger.warning(
                    "Quick Ratio calculation: Missing current_assets or current_liabilities data"
                )
                return None
        ...
        try:
            ...
            if cash is None or pd.isna(cash):
                ...
                if other_assets is not None and not pd.isna(other_assets):
                    cash = other_assets
                    logger.debug(
                        "Cash Ratio calculation: Using 'other_assets' as proxy for cash"
                    )
            ...
            if cash is None or pd.isna(cash):
                logger.warning(
                    "Cash Ratio calculation: Cash and cash equivalents data not available in schema"
                )
                return None

            if current_liabilities is None or pd.isna(current_liabilities):
                logger.warning(
                    "Cash Ratio calculation: Missing current_liabilities data"
                )
                return None
            ...
            logger.error(
                f"Cash Ratio calculation failed - IndexError (empty dataframe): {str(e)}"
            )
            return None
        except Exception as e:
