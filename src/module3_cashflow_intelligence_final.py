...
Final version that reimplements necessary functions from cashflow_kpis.py and cagr.py
to bypass import issues with src.analytics.__init__.py
"""
...
# =============================================================================
# REIMPLEMENTED FUNCTIONS FROM casflow_kpis.py
# =============================================================================
...
def _safe_get_value(df: pd.DataFrame, column: str, default: Any = None) -> Any:
    ...
    # Try to get the column, return default if not found
...
def _get_capital_expenditure(cf_data: Dict[str, Any]) -> float:
    ...
    # If not available, try alternative column name
    if capex == 0:  # Only try alternative if primary didn't give us a value
        capex = _safe_get_value(cf_data, CAPEX_ALT_COLUMN, 0)
...
def calculate_free_cash_flow(cf_data: Dict[str, Any]) -> Optional[float]:
    ...
    if not _validate_numeric(ocf, "operating_cash_flow"):
        return None
