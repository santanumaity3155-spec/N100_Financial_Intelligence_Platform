...
Workaround version that bypasses the problematic __init__.py file by
directly loading the required modules using importlib.
"""
...
calculate_free_cash_flow = cashflow_kpis.calculate_free_cash_flow
_get_operating_cash_flow = cashflow_kpis._get_operating_cash_flow
_safe_get_value = cashflow_kpis._safe_get_value
classify_capital_allocation = cashflow_kpis.classify_capital_allocation
RATING_EXCELLENT = cashflow_kpis.RATING_EXCELLENT
RATING_GOOD = cashflow_kpis.RATING_GOOD
RATING_MODERATE = cashflow_kpis.RATING_MODERATE
RATING_WEAK = cashflow_kpis.RATING_WEAK
RATING_DISTRESSED = cashflow_kpis.RATING_DISTRESSED

calculate_cagr = cagr_module.calculate_cagr
FLAG_NORMAL = cagr_module.FLAG_NORMAL
FLAG_ZERO_BASE = cagr_module.FLAG_ZERO_BASE
FLAG_DECLINE_TO_LOSS = cagr_module.FLAG_DECLINE_TO_LOSS
FLAG_TURNAROUND = cagr_module.FLAG_TURNAROUND
FLAG_BOTH_NEGATIVE = cagr_module.FLAG_BOTH_NEGATIVE
...
def compute_cfo_quality(cf_data_list, pl_data_list):
    ...
    for cf_data, pl_data in zip(cf_data_list, pl_data_list):
        cfo = cf_data.get('cash_from_operating_activity') or cf_data.get('operating_activity')
...
def compute_capex_intensity_latest(cf_data, pl_data):
    ...
    investing_activity = abs(cf_data.get('cash_from_investing_activity') or cf_data.get('investing_activity') or 0)
...
def compute_fcf_cagr_5yr(cf_data_list, pl_data_list):
    ...
    fcf_values = []
    for cf_data, pl_data in zip(cf_data_list, pl_data_list):
        ocf = cf_data.get('cash_from_operating_activity') or cf_data.get('operating_activity') or 0
        capex = abs(cf_data.get('cash_from_investing_activity') or cf_data.get('investing_activity') or 0)
        fcf = ocf - capex
...
def main():
    ...
    logger.info("Starting Module 3: Cash Flow Intelligence Engine (Workaround Version)")
