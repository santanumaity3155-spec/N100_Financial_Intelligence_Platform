...
class ValuationYearRecord(BaseModel):
    """ValuationYearRecord class representation."""
...
class CompanyValuationResponse(BaseModel):
    """CompanyValuationResponse class representation."""

    ticker: str = Field(..., description="Company ticker symbol")
    company_name: str = Field(..., description="Official company name")
    historical_valuation: List[ValuationYearRecord] = Field(
        ..., description="Chronological annual valuation records (2019 to 2024)"
    )
    ...
    try:
        ...
        if not mc_df.empty:
            for _, row in mc_df.iterrows():
                ...
                ev_cap_v = (
                    float(row["enterprise_value"])
                    if pd.notna(row["enterprise_value"])
                    else None
                )
        ...
        return CompanyValuationResponse(
            ticker=ticker_clean, company_name=comp_name, historical_valuation=records
        )
