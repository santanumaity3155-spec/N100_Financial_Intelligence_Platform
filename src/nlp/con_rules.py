...
class CON_02(FinancialRule):
    ...
    def evaluate(self, context: Any, conn: Optional[Any] = None) -> RuleResult:
        ...
        if len(history) < 3:
            return _untriggered(
                self, context, f"Insufficient FCF history ({len(history)} years)."
            )
...
class CON_03(FinancialRule):
    ...
    def evaluate(self, context: Any, conn: Optional[Any] = None) -> RuleResult:
        ...
        if len(history) < 4:
            return _untriggered(
                self, context, f"Insufficient OPM history ({len(history)} years)."
            )
...
class CON_04(FinancialRule):
    ...
    def evaluate(self, context: Any, conn: Optional[Any] = None) -> RuleResult:
        ...
        return _untriggered(
            self, context, f"Latest Net Profit = {net_profit:,.0f} (>= 0)"
        )
...
class CON_05(FinancialRule):
    ...
    def evaluate(self, context: Any, conn: Optional[Any] = None) -> RuleResult:
        ...
        if len(history) < 3:
            return _untriggered(
                self, context, f"Insufficient revenue history ({len(history)} years)."
            )
...
class CON_08(FinancialRule):
    ...
    def evaluate(self, context: Any, conn: Optional[Any] = None) -> RuleResult:
        ...
        if len(history) < 4:
            return _untriggered(
                self, context, f"Insufficient D/E history ({len(history)} years)."
            )
...
class CON_09(FinancialRule):
    ...
    def evaluate(self, context: Any, conn: Optional[Any] = None) -> RuleResult:
        ...
        if len(history) < 4:
            return _untriggered(
                self, context, f"Insufficient EPS history ({len(history)} years)."
            )
...
class CON_11(FinancialRule):
    ...
    def evaluate(self, context: Any, conn: Optional[Any] = None) -> RuleResult:
        ...
        if ebitda <= 0:
            return _untriggered(
                self, context, f"EBITDA is not positive ({ebitda:,.0f})."
            )

        ratio = safe_divide(net_debt, ebitda)
        if ratio is None:
            return _untriggered(
                self, context, "Could not calculate Net Debt / EBITDA ratio."
            )
