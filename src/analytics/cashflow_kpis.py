...
def _validate_time_series(
    df: pd.DataFrame, period_column: str = PERIOD_COLUMN
) -> Optional[List[str]]:
    """
    ...
    if period_column not in df.columns:
        logger.warning(
            f"Time series validation: Period column '{period_column}' not found"
        )
        return None
...
def calculate_free_cash_flow(cf_data: pd.DataFrame) -> Optional[float]:
    ...
    try:
        ...
        logger.debug(
            f"Free Cash Flow calculated: {fcf:.2f} (OCF={ocf:.2f}, CapEx={capex:.2f})"
        )
        return round(fcf, 2)
...
def calculate_fcf_margin(
    cf_data: pd.DataFrame, pl_data: pd.DataFrame
) -> Optional[float]:
    """
...
def calculate_cash_conversion(
    ...
    if net_profit <= 0:
        logger.warning(
            f"Cash Conversion calculation: Net profit is {net_profit} (<= 0)"
        )
        return {"value": None, "flag": "INVALID_PROFIT"}
...
def calculate_capex_intensity(cf_data: pd.DataFrame) -> Optional[float]:
    ...
    if not _validate_dataframe(
        cf_data, [OCF_COLUMN, CAPEX_COLUMN], " for CapEx Intensity"
    ):
        return None
...
def calculate_cash_reinvestment_ratio(cf_data: pd.DataFrame) -> Optional[float]:
    ...
    if not _validate_dataframe(
        cf_data, [OCF_COLUMN, CAPEX_COLUMN], " for Cash Reinvestment Ratio"
    ):
        return None
...
def calculate_cash_return_on_assets(
    cf_data: pd.DataFrame, bs_data: pd.DataFrame
) -> Optional[float]:
    """
...
def calculate_operating_cashflow_growth(
    ...
    for window in CAGR_WINDOWS:
        cagr_result = _calculate_cagr_for_window(
            ocf_data, OCF_COLUMN, window, f"ocf_{window}yr", company_id, period
        )
...
def _get_empty_cagr_result(metric_type: str) -> Dict[str, Any]:
    ...
    results = {}
    for window in CAGR_WINDOWS:
        results[f"{metric_type}_cagr_{window}yr"] = {
            "value": None,
            "flag": "INSUFFICIENT",
        }
...
def calculate_all_cashflow_kpis(
    ...
    """
    logger.info(
        f"Calculating all cash flow KPIs for company_id={company_id}, period={period}"
    )

    results = {"company_id": company_id, "period": period}
    ...
    rating = classify_capital_allocation(
        fcf, cash_conv_result["value"], capex_intensity, ocf
    )
    ...
    logger.info(
        f"Calculated {calculated_count} cash flow KPIs for {company_id}, period {period}"
    )
        ...
        with open(output_path, "w", newline="", encoding="utf-8") as csvfile:
            ...
            for result in results:
                row = {
                    ...
                    "cash_reinvestment_ratio": result.get(
                        "cash_reinvestment_ratio", ""
                    ),
                    "operating_cashflow_growth": (
                        result.get("ocf_cagr_3yr", {}).get("value", "")
                        if result.get("ocf_cagr_3yr")
                        else ""
                    ),
                    "capital_allocation_rating": result.get(
                        "capital_allocation_rating", ""
                    ),
                }
                writer.writerow(row)
