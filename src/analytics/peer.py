        ...
        if source == "excel":
            ...
            if not excel_path.exists():
                raise FileNotFoundError(
                    f"Peer groups Excel file not found: {excel_path}"
                )
...
def assign_peer_groups(
    df: pd.DataFrame, company_id_col: str = "company_id"
) -> pd.DataFrame:
    """
...
def export_percentiles(
    df: pd.DataFrame, output_path: Optional[Path] = None, period: Optional[str] = None
) -> Path:
    """
    ...
    try:
        ...
        percentile_cols = [
            ...
            if f"{m}_percentile" in export_df.columns
        ]
                ...
                export_records.append(
                    {
                        ...
                        "metric": metric,
                        "metric_value": (
                            metric_value if not pd.isna(metric_value) else None
                        ),
        ...
        export_df_final = export_df_final.sort_values(
            ["peer_group", "metric", "percentile_rank"], ascending=[True, True, False]
        )
...
def get_peer_summary(df: pd.DataFrame) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(
            f"Summary generated: {summary['total_companies']} companies processed"
        )
        return summary
    ...
    try:
        ...
        if "peer_group_name" in df.columns:
            ...
            invalid_groups = [
                ...
                if g not in SUPPORTED_PEER_GROUPS and g != "No peer group assigned"
            ]
            ...
            results["checks"].append(
                {
                    ...
                    "status": "passed",
                    "unique_groups": len(
                        [g for g in peer_groups if g != "No peer group assigned"]
                    ),
                }
            )
        ...
        percentile_cols = [
            f"{m}_percentile"
            for m in SUPPORTED_METRICS
            if f"{m}_percentile" in df.columns
        ]

        if len(percentile_cols) != len(SUPPORTED_METRICS):
            missing_metrics = [
                m for m in SUPPORTED_METRICS if f"{m}_percentile" not in df.columns
            ]
            results["warnings"].append(
                {
...
class PeerPercentileEngine:
    ...
    def __init__(self, output_dir: Path = OUTPUT_DIR, source: str = "database"):
        """
        Initialize pipeline.
    ...
    def run(
        ...
        try:
            ...
            if not validation_results["valid"]:
                ...
                self.pipeline_stats["validation_errors"] = len(
            ...
            self.pipeline_stats["companies_with_peer_group"] = (
                (df["peer_group_name"] != "No peer group assigned").sum()
                if "peer_group_name" in df.columns
                else 0
            )
            ...
            self.pipeline_stats["metrics_processed"] = (
                len(metrics) if metrics else len(SUPPORTED_METRICS)
            )
...
def validate_database_integrity() -> Dict[str, Any]:
    ...
    try:
        ...
        if missing_peer_group > 0:
            ...
            results["checks"].append(
                {"check": "missing_peer_group", "status": "passed"}
            )
        ...
        if invalid_percentiles > 0:
            ...
            results["checks"].append(
                {"check": "invalid_percentiles", "status": "passed"}
            )
