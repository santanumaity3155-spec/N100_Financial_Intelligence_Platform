...
def load_percentile_data(
    ...
    """
    logger.info(
        f"Loading percentile data: company={company_id}, peer_group={peer_group}, period={period}"
    )
    ...
    try:
        ...
        for metric in SUPPORTED_METRICS:
            ...
            if metric_data.empty:
                logger.warning(
                    f"No data for metric {metric} in peer group {peer_group}"
                )
...
def prepare_radar_data(company_id: str, period: Optional[str] = None) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if df.empty:
            raise MetricValidationError(
                f"No percentile data found for company: {company_id}"
            )
...
def validate_chart_inputs(
    company_percentiles: Dict[str, float], benchmark: Dict[str, float], peer_group: str
) -> bool:
    """
    ...
    invalid_metrics = [
        m for m in company_percentiles.keys() if m not in SUPPORTED_METRICS
    ]
    if invalid_metrics:
    ...
    for metric, percentile in benchmark.items():
        if not 0 <= percentile <= 1:
            errors.append(
                f"Benchmark percentile out of range for {metric}: {percentile}"
            )
...
def generate_radar_chart(
    ...
    try:
        ...
        # Create figure
        fig, ax = plt.subplots(
            figsize=CHART_FIGSIZE, subplot_kw=dict(projection="polar")
        )
        ...
        # Save chart
        plt.savefig(
            output_path, dpi=CHART_DPI_HIGH_RES, bbox_inches="tight", facecolor="white"
        )
        plt.close(fig)
    ...
    try:
        ...
        if not chart_files:
            ...
            results["checks"].append(
                {"check": "charts_exist", "status": "passed", "count": len(chart_files)}
            )
        ...
        if invalid_charts:
            results["errors"].append(
                {"error": "invalid_png_files", "files": invalid_charts}
            )
            results["valid"] = False
...
class RadarChartEngine:
    ...
    def __init__(
        self, output_dir: Path = RADAR_CHARTS_DIR, period: Optional[str] = None
    ):
        """
        Initialize pipeline.
    ...
    def run(self, batch_size: int = 50) -> Dict[str, Any]:
        ...
        try:
            ...
            stats = generate_all_charts(
                period=self.period, output_dir=self.output_dir, batch_size=batch_size
            )
