...
SECTOR_OVERRIDES = {"ULTRACEMCO": "Cement", "UNIONBANK": "Public Sector Banks"}
            ...
            results.append(
                {
                    ...
                    "debt_to_equity": (
                        float(de_val)
                        if de_val is not None and not pd.isna(de_val)
                        else np.nan
                    ),
                    "revenue_cagr_5yr": (
                        float(rev_cagr)
                    ...
                    "fcf_cagr_5yr": (
                        float(fcf_cagr)
                        if fcf_cagr is not None and not pd.isna(fcf_cagr)
                        else np.nan
                    ),
                    "operating_profit_margin_pct": (
                        float(opm_val)
                        if opm_val is not None and not pd.isna(opm_val)
                        else np.nan
                    ),
                }
            )
...
def impute_missing_values(
    df: pd.DataFrame, features: List[str] = REQUIRED_FEATURES
) -> pd.DataFrame:
    """
    ...
    for feat in features:
        ...
        if missing_cnt > 0:
            logger.info(
                f"Imputing {missing_cnt} missing values for feature '{feat}'..."
            )
    ...
    if np.isnan(feature_matrix).any():
        raise ValueError(
            "Feature matrix contains unresolved NaN values after imputation."
        )
...
def scale_features(
    ...
    logger.info(
        f"StandardScaler fit and transformed {X_scaled.shape[0]} rows across {X_scaled.shape[1]} features."
    )
...
def run_kmeans(
    ...
    if X_scaled.shape[0] < n_clusters:
        raise ValueError(
            f"Number of samples ({X_scaled.shape[0]}) is less than n_clusters ({n_clusters})."
        )

    kmeans = KMeans(n_clusters=n_clusters, random_state=random_state, n_init=n_init)
    kmeans.fit(X_scaled)
    logger.info(
        f"KMeans fitted with n_clusters={n_clusters}, random_state={random_state}, inertia={kmeans.inertia_:.4f}."
    )
    return kmeans
...
def generate_elbow_plot(
    ...
    plt.plot(
        ks, inertias, "o-", color="#1f77b4", linewidth=2, markersize=7, label="Inertia"
    )

    if highlight_k in inertia_dict:
        plt.scatter(
            [highlight_k],
            [inertia_dict[highlight_k]],
            color="#d62728",
            s=120,
            zorder=5,
    ...
    plt.title(
        "KMeans Clustering — Elbow Method (Inertia vs k)",
        fontsize=14,
        fontweight="bold",
        pad=12,
    )
    ...
    plt.ylabel(
        "Inertia (Within-Cluster Sum of Squares)", fontsize=11, fontweight="semibold"
    )
    plt.xticks(ks)
...
def run_kmeans_clustering(
    ...
    distances = compute_centroid_distances(
        X_scaled, kmeans.labels_, kmeans.cluster_centers_
    )

    # Step 6: Compute elbow curve and save plot
    inertia_dict = compute_elbow_inertia(X_scaled, random_state=random_state)
    generate_elbow_plot(
        inertia_dict, output_path=elbow_plot_path, highlight_k=n_clusters
    )

    # Step 7: Export cluster labels CSV
    out_df = generate_cluster_output(
        imputed_df, kmeans.labels_, distances, output_path=output_csv_path
    )
...
if __name__ == "__main__":
    ...
    print(
        "Execution complete. Output CSV generated with",
        len(results["cluster_labels_df"]),
        "rows.",
    )
