...
def validate_company_period(
    ...
    for field in critical_fields:
        ...
        if value is not None and (
            pd.isna(value) or (isinstance(value, float) and (value != value))
        ):
            errors.append(
                ValidationError(
                    str(company_id),
...
def validate_financial_data_availability(
    ...
    if pl_data.empty:
        errors.append(
            ValidationError(
                company_id, period, "MISSING_PL", "Profit & Loss data is missing"
            )
        )
    ...
    if bs_data.empty:
        errors.append(
            ValidationError(
                company_id, period, "MISSING_BS", "Balance Sheet data is missing"
            )
        )
    ...
    if cf_data.empty:
        errors.append(
            ValidationError(
                company_id, period, "MISSING_CF", "Cash Flow data is missing"
            )
        )
...
def insert_financial_ratios(record: Dict[str, Any]) -> Tuple[bool, Optional[str]]:
    ...
    try:
        ...
        logger.error(
            f"Failed to insert record for {record.get('company_id')}, {record.get('period')}: {error_msg}"
        )
    ...
    try:
        ...
        if pl_df.empty:
            ...
            result["validation_errors"].append(
                {"error_type": "NO_DATA", "message": "No P&L data available"}
            )
            return result
        ...
        if len(periods) == 0:
            result["status"] = "failed"
            result["validation_errors"].append(
                {"error_type": "NO_PERIODS", "message": "No periods found in P&L data"}
            )
            return result
        ...
        for period in periods:
            try:
                ...
                pl_period = (
                    pl_df[pl_df["period"] == period]
                    if "period" in pl_df.columns
                    else pl_df
                )
                bs_period = (
                    bs_df[bs_df["period"] == period]
                    if "period" in bs_df.columns
                    else bs_df
                )
                cf_period = (
                    cf_df[cf_df["period"] == period]
                    if "period" in cf_df.columns
                    else cf_df
                )
                ...
                if validation_errors:
                    ...
                    logger.warning(
                        f"Validation errors for {company_id}, {period}: {len(validation_errors)} errors"
                    )
                    continue
                ...
                cashflow_data = calculate_all_cashflow_kpis(
                    cf_period, pl_period, bs_period, company_id, period
                )
                ...
                if record_errors:
                    result["validation_errors"].extend(
                        [e.to_dict() for e in record_errors]
                    )
                    logger.warning(
                        f"Record validation errors for {company_id}, {period}: {len(record_errors)} errors"
                    )
                    continue
                ...
                logger.error(
                    f"Error processing period {period} for {company_id}: {str(e)}"
                )
                result["validation_errors"].append(
                    {"error_type": "PROCESSING_ERROR", "message": str(e)}
                )
        ...
        result["status"] = "failed"
        result["validation_errors"].append(
            {"error_type": "FATAL_ERROR", "message": str(e)}
        )
...
class RatioEnginePipeline:
    ...
    def _process_single_company(
        ...
        if result["validation_errors"]:
            self.pipeline_stats["validation_failures"] += len(
            ...
            for record in records:
                try:
                    ...
                    if success:
                        ...
                        logger.info(
                            f"Successfully inserted {company_id}, {record['period']}"
                        )
                    else:
                        conn.rollback()
                        self.pipeline_stats["rows_skipped"] += 1
                        logger.error(
                            f"Failed to insert {company_id}, {record['period']}: {error_msg}"
                        )
    ...
    try:
        ...
        if duplicates:
            ...
            results["checks"].append(
                {"check": "duplicates", "status": "failed", "count": len(duplicates)}
            )
        else:
            results["checks"].append({"check": "duplicates", "status": "passed"})
        ...
        if missing_company_id > 0:
            ...
            results["checks"].append(
                {"check": "missing_company_id", "status": "passed"}
            )
        ...
        if missing_period > 0:
            results["valid"] = False
            results["checks"].append(
                {"check": "missing_period", "status": "failed", "count": missing_period}
            )
        else:
            results["checks"].append({"check": "missing_period", "status": "passed"})
