...
EXCEL_FAIR_FILL = PatternFill(
    start_color="90EE90", end_color="90EE90", fill_type="solid"
)  # Green
EXCEL_DISCOUNT_FILL = PatternFill(
    start_color="FFFF00", end_color="FFFF00", fill_type="solid"
)  # Yellow
EXCEL_CAUTION_FILL = PatternFill(
...
def _get_latest_period(
    conn: sqlite3.Connection, table: str, company_id: str
) -> Optional[str]:
    """
    ...
    try:
        ...
        logger.warning(
            f"Failed to get latest period for {company_id} from {table}: {str(e)}"
        )
        return None
...
def calculate_sector_median_pe(df: pd.DataFrame) -> pd.DataFrame:
    ...
    try:
        ...
        sector_medians = (
            valid_pe.groupby("broad_sector")["pe_ratio"].median().reset_index()
        )
...
def calculate_sector_relative_pe(
    company_pe: Optional[float], sector_median_pe: Optional[float]
) -> Optional[float]:
    """
    ...
    try:
        ...
        if comp_pe is None or sec_med_pe is None:
            logger.debug(
                "Sector Relative PE calculation: Missing PE or Sector Median PE"
            )
            return None
        ...
        logger.debug(
            f"Sector Relative PE calculated: {relative_pe:.2f} (Company PE={comp_pe}, Sector Median={sec_med_pe})"
        )
...
def build_valuation_dataframe() -> pd.DataFrame:
    ...
    try:
        ...
        valuation_df["pe_vs_sector_median_pct"] = valuation_df.apply(
            lambda row: (
                round((row["pe_ratio"] / row["sector_median_pe"]) * 100, 2)
                if pd.notna(row["pe_ratio"])
                and pd.notna(row["sector_median_pe"])
                and row["sector_median_pe"] > 0
                else None
            ),
            axis=1,
        )
        ...
        valuation_df["valuation_flag"] = valuation_df["pe_vs_sector_median_pct"].apply(
...
def export_valuation_summary(
    df: pd.DataFrame, output_path: Path = VALUATION_SUMMARY_XLSX
) -> Path:
    """
    ...
    try:
        ...
        elapsed_time = time.time() - start_time
        logger.info(
            f"Excel exported successfully to {output_path} in {elapsed_time:.2f}s"
        )
...
def export_valuation_flags(
    df: pd.DataFrame, output_path: Path = VALUATION_FLAGS_CSV
) -> Path:
    """
    ...
    try:
        ...
        if "PE" in flags_df.columns and "Sector Median PE" in flags_df.columns:
            flags_df["Difference %"] = flags_df.apply(
                lambda row: (
                    round(
                        (
                            (row["PE"] - row["Sector Median PE"])
                            / row["Sector Median PE"]
                        )
                        * 100,
                        2,
                    )
                    if pd.notna(row["PE"])
                    and pd.notna(row["Sector Median PE"])
                    and row["Sector Median PE"] > 0
                    else None
                ),
                axis=1,
            )
        ...
        flags_df = flags_df.sort_values(
            ["Valuation Flag", "Sector"], ascending=[True, True]
        )
        ...
        elapsed_time = time.time() - start_time
        logger.info(
            f"CSV exported successfully to {output_path} in {elapsed_time:.2f}s"
        )
