...
def _validate_time_series(
    df: pd.DataFrame, period_column: str = "period"
) -> Optional[List[str]]:
    """
...
def calculate_revenue_cagr(
    ...
    """
    logger.info(
        f"Calculating Revenue CAGR for company_id={company_id}, period={period}"
    )
...
def calculate_pat_cagr(
    ...
    for window in CAGR_WINDOWS:
        cagr_result = _calculate_cagr_for_window(
            pat_data, PAT_COLUMN, window, f"pat_{window}yr", company_id, period
        )
...
def calculate_eps_cagr(
    ...
    for window in CAGR_WINDOWS:
        cagr_result = _calculate_cagr_for_window(
            eps_data, EPS_COLUMN, window, f"eps_{window}yr", company_id, period
        )
...
def calculate_all_cagr(
    ...
    """
    logger.info(
        f"Calculating all CAGR metrics for company_id={company_id}, period={period}"
    )
    ...
    logger.info(
        f"Calculated {calculated_count} CAGR metrics for {company_id}, period {period}"
    )
