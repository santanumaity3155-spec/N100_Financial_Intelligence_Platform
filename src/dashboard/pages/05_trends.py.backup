...
from src.dashboard.utils.db import get_companies, get_ratios, get_pl, get_cf, get_bs
...
st.set_page_config(
    page_title="Trend Analysis - N100 Financial Intelligence",
    page_icon="📈",
    layout="wide",
...
# (display_name, source_table, column_name, format_string)
TREND_METRICS: List[Tuple[str, str, str, str]] = [
    ("Revenue", "profit_loss", "revenue", "{:,.0f}"),
    ("Net Profit", "profit_loss", "net_profit", "{:,.0f}"),
    ("ROE", "financial_ratios", "roe", "{:.2f}%"),
    ("ROCE", "financial_ratios", "roce", "{:.2f}%"),
    ("Debt to Equity", "financial_ratios", "debt_to_equity", "{:.2f}"),
    ("Operating Profit Margin", "financial_ratios", "operating_profit_margin", "{:.2f}%"),
...
def render_sidebar(companies_df: pd.DataFrame) -> Tuple[Optional[str], Optional[str], Optional[List[int]]]:
    """
    ...
    Tuple[Optional[str], Optional[str], Optional[List[int]]]
        (selected_company_id, selected_company_name, selected_years)
    """
