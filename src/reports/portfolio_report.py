...
class PortfolioReportGenerator:
        ...
        try:
            with self._get_db_conn() as conn:
                comp_df = pd.read_sql_query(
                    "SELECT company_id, company_name, sector, industry FROM companies ORDER BY company_id ASC",
                    conn,
                )
                ...
                for _, r in merged.iterrows():
                    ...
                    # Year check
                    pl_c = conn.execute(
                        "SELECT COUNT(DISTINCT period) FROM profit_loss WHERE company_id = ?",
                        (cid,),
                    ).fetchone()[0]
                    bs_c = conn.execute(
                        "SELECT COUNT(DISTINCT period) FROM balance_sheet WHERE company_id = ?",
                        (cid,),
                    ).fetchone()[0]
                    cf_c = conn.execute(
                        "SELECT COUNT(DISTINCT period) FROM cash_flow WHERE company_id = ?",
                        (cid,),
                    ).fetchone()[0]
    ...
    def _calculate_trend(
        self, curr_val: float, prev_val: float, inverted: bool = False
    ) -> Tuple[str, str, str]:
        """
        ...
        if not inverted:
            ...
            if pct_change < -2.0:
                return (
                    ...
                    f"{pct_change:+.1f}%",
                    "#15803D",
                )  # Green UP (improved/reduced debt)
            else:
                return (
                    "↓",
                    f"{pct_change:+.1f}%",
                    "#B91C1C",
                )  # Red DOWN (increased debt)
    ...
    def generate(self, output_path: Optional[Path] = None) -> Path:
        ...
        with self._get_db_conn() as conn:
            for idx, comp in enumerate(companies_list):
                ...
                hdr_tbl = Table(
                    [
                        [Paragraph(hdr_text, title_style)],
                ...
                pl_df = pd.read_sql_query(
                    "SELECT period, sales, net_profit, opm_percentage FROM profit_loss WHERE company_id = ? ORDER BY period DESC LIMIT 2",
                    conn,
                    params=[cid],
                )
                kpi_df = pd.read_sql_query(
                    "SELECT period, roe, roce, debt_to_equity FROM financial_kpis WHERE company_id = ? ORDER BY period DESC LIMIT 2",
                    conn,
                    params=[cid],
                )
                hs_df = pd.read_sql_query(
                ...
                curr_opm = (
                    pl_df.iloc[0]["opm_percentage"] if len(pl_df) >= 1 else np.nan
                )
                prev_opm = (
                    pl_df.iloc[1]["opm_percentage"] if len(pl_df) >= 2 else np.nan
                )
                ...
                curr_de = (
                    kpi_df.iloc[0]["debt_to_equity"] if len(kpi_df) >= 1 else np.nan
                )
                prev_de = (
                    kpi_df.iloc[1]["debt_to_equity"] if len(kpi_df) >= 2 else np.nan
                )

                # Trends
                arr_rev, chg_rev, clr_rev = self._calculate_trend(
                    curr_sales, prev_sales
                )
                ...
                arr_roce, chg_roce, clr_roce = self._calculate_trend(
                    curr_roce, prev_roce
                )
                arr_de, chg_de, clr_de = self._calculate_trend(
                    curr_de, prev_de, inverted=True
                )
                arr_opm, chg_opm, clr_opm = self._calculate_trend(curr_opm, prev_opm)

                elements.append(
                    Paragraph(
                        "Top 6 Key Financial Metrics & Year-over-Year Trends",
                        section_heading,
                    )
                )
                elements.append(
                    HRFlowable(
                        width="100%", thickness=1, color=PRIMARY_NAVY, spaceAfter=8
                    )
                )
                ...
                metrics_specs = [
                    (
                        "Revenue (Sales)",
                        f"₹{curr_sales:,.0f} Cr" if pd.notna(curr_sales) else "N/A",
                        f"₹{prev_sales:,.0f} Cr" if pd.notna(prev_sales) else "N/A",
                        chg_rev,
                        arr_rev,
                        clr_rev,
                    ),
                    (
                        "Net Profit (PAT)",
                        f"₹{curr_pat:,.0f} Cr" if pd.notna(curr_pat) else "N/A",
                        f"₹{prev_pat:,.0f} Cr" if pd.notna(prev_pat) else "N/A",
                        chg_pat,
                        arr_pat,
                        clr_pat,
                    ),
                    (
                        ...
                        f"{curr_roe:.2f}%" if pd.notna(curr_roe) else "N/A",
                        f"{prev_roe:.2f}%" if pd.notna(prev_roe) else "N/A",
                        chg_roe,
                        arr_roe,
                        clr_roe,
                    ),
                    (
                        ...
                        f"{curr_roce:.2f}%" if pd.notna(curr_roce) else "N/A",
                        f"{prev_roce:.2f}%" if pd.notna(prev_roce) else "N/A",
                        chg_roce,
                        arr_roce,
                        clr_roce,
                    ),
                    (
                        "Debt-to-Equity (x)",
                        f"{curr_de:.2f}x" if pd.notna(curr_de) else "N/A",
                        f"{prev_de:.2f}x" if pd.notna(prev_de) else "N/A",
                        chg_de,
                        arr_de,
                        clr_de,
                    ),
                    (
                        ...
                        f"{curr_opm:.2f}%" if pd.notna(curr_opm) else "N/A",
                        f"{prev_opm:.2f}%" if pd.notna(prev_opm) else "N/A",
                        chg_opm,
                        arr_opm,
                        clr_opm,
                    ),
                ...
                for name, c_val, p_val, chg, arr, clr in metrics_specs:
                    arr_p = Paragraph(
                        f"<font color='{clr}'><b>{arr} {chg}</b></font>", cell_style
                    )
                    table_data.append(
                        [
                ...
                kpi_table = Table(
                    table_data,
                    colWidths=[c_w * 1.4, c_w * 0.9, c_w * 0.9, c_w * 0.9, c_w * 0.9],
                )
                kpi_table.setStyle(
                    TableStyle(
                        [
                ...
                score_val = (
                    hs_df.iloc[0]["overall_score"]
                    if not hs_df.empty and pd.notna(hs_df.iloc[0]["overall_score"])
                    else np.nan
                )
                rating_val = (
                    hs_df.iloc[0]["rating"]
                    if not hs_df.empty and pd.notna(hs_df.iloc[0]["rating"])
                    else "N/A"
                )
...
def generate_portfolio_summary_report(
    output_path: Optional[Path] = None, db_path: Optional[Path] = None
) -> Path:
    """Generate portfolio summary report functionality."""
