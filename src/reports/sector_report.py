...
class SectorReportGenerator:
    ...
    def _load_sector_companies(self) -> pd.DataFrame:
        ...
        try:
            with self._get_db_conn() as conn:
                comp_df = pd.read_sql_query("SELECT company_id, company_name, sector, industry FROM companies", conn)
                ...
                merged["broad_sector"] = merged["sub_sector"].map(SUB_SECTOR_TO_BROAD_SECTOR).fillna("Capital Goods & Engineering")
                
                # Filter to sector
                sector_comps = merged[merged["broad_sector"].str.upper() == self.sector_name.upper()].copy()
                if sector_comps.empty:
                    # Fallback to sub_sector or raw sector matching
                    sector_comps = merged[merged["sub_sector"].str.upper() == self.sector_name.upper()].copy()
                ...
                for _, row in sector_comps.iterrows():
                    ...
                    # Latest P&L
                    pl = pd.read_sql_query("SELECT sales, net_profit, opm_percentage FROM profit_loss WHERE company_id = ? ORDER BY period DESC LIMIT 1", conn, params=[cid])
                    sales = pl.iloc[0]["sales"] if not pl.empty and pd.notna(pl.iloc[0]["sales"]) else np.nan
                    pat = pl.iloc[0]["net_profit"] if not pl.empty and pd.notna(pl.iloc[0]["net_profit"]) else np.nan
                    opm = pl.iloc[0]["opm_percentage"] if not pl.empty and pd.notna(pl.iloc[0]["opm_percentage"]) else np.nan
                    
                    # Latest KPIs
                    kpi = pd.read_sql_query("SELECT roe, roce, debt_to_equity, interest_coverage FROM financial_kpis WHERE company_id = ? ORDER BY period DESC LIMIT 1", conn, params=[cid])
                    roe = kpi.iloc[0]["roe"] if not kpi.empty and pd.notna(kpi.iloc[0]["roe"]) else np.nan
                    roce = kpi.iloc[0]["roce"] if not kpi.empty and pd.notna(kpi.iloc[0]["roce"]) else np.nan
                    de = kpi.iloc[0]["debt_to_equity"] if not kpi.empty and pd.notna(kpi.iloc[0]["debt_to_equity"]) else np.nan
                    ic = kpi.iloc[0]["interest_coverage"] if not kpi.empty and pd.notna(kpi.iloc[0]["interest_coverage"]) else np.nan
                    
                    # Health score
                    hs = pd.read_sql_query("SELECT overall_score, rating FROM financial_health_scores WHERE company_id = ? ORDER BY period DESC LIMIT 1", conn, params=[cid])
                    score = hs.iloc[0]["overall_score"] if not hs.empty and pd.notna(hs.iloc[0]["overall_score"]) else np.nan
                    rating = hs.iloc[0]["rating"] if not hs.empty and pd.notna(hs.iloc[0]["rating"]) else "N/A"
    ...
    def generate(self, output_path: Optional[Path] = None) -> Path:
        ...
        sub_text = f"Total Companies: {company_cnt} | N100 Financial Intelligence Universe"
        
        header_table = Table([[Paragraph(header_text, title_style)], [Paragraph(sub_text, subtitle_style)]], colWidths=[printable_width])
        ...
        if self.company_data.empty:
            elements.append(Paragraph("<i>No company data available for this sector.</i>", body_style))
            doc.build(elements, canvasmaker=NumberedCanvas)
            return output_path

        # 2. Sector Overview & Median KPIs
        elements.append(Paragraph("Sector Overview & Median Benchmarks", section_heading))
        elements.append(HRFlowable(width="100%", thickness=1, color=PRIMARY_NAVY, spaceAfter=6))
        ...
        median_tiles_data = [
            ...
            [
                Paragraph(f"₹{med_rev:,.0f} Cr" if pd.notna(med_rev) else "N/A", table_cell_style),
                Paragraph(f"₹{med_pat:,.0f} Cr" if pd.notna(med_pat) else "N/A", table_cell_style),
                Paragraph(f"{med_opm:.2f}%" if pd.notna(med_opm) else "N/A", table_cell_style),
                Paragraph(f"{med_roe:.2f}%" if pd.notna(med_roe) else "N/A", table_cell_style),
            ],
            [
            ...
            [
                Paragraph(f"{med_roce:.2f}%" if pd.notna(med_roce) else "N/A", table_cell_style),
                Paragraph(f"{med_de:.2f}x" if pd.notna(med_de) else "N/A", table_cell_style),
                Paragraph(f"{med_ic:.2f}x" if pd.notna(med_ic) else "N/A", table_cell_style),
                Paragraph(f"{med_score:.1f}/100" if pd.notna(med_score) else "N/A", table_cell_style),
            ]
        ]
        ...
        elements.append(Paragraph("Company Financial Performance Matrix (Eight Key Metrics)", section_heading))
        elements.append(HRFlowable(width="100%", thickness=1, color=PRIMARY_NAVY, spaceAfter=6))
        ...
        for _, r in df.sort_values(by="company_id").iterrows():
            ...
            ic_str = f"{r['interest_coverage']:.1f}" if pd.notna(r['interest_coverage']) else "N/A"
            hs_str = f"{r['health_score']:.1f}" if pd.notna(r['health_score']) else "N/A"
...
def generate_sector_report(sector_name: str, output_path: Optional[Path] = None, db_path: Optional[Path] = None) -> Path:
