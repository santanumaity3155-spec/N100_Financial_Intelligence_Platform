...
class CompanyTearsheetGenerator:
    ...
    def _load_cf_data(self) -> pd.DataFrame:
        try:
            with self._get_db_conn() as conn:
                query = (
                    "SELECT * FROM cash_flow WHERE company_id = ? ORDER BY period ASC"
                )
                df = pd.read_sql_query(query, conn, params=[self.company_id])
                return df
        except Exception as e:
    ...
    def _load_pros_cons(self) -> Dict[str, List[str]]:
        ...
        if csv_path.exists():
            try:
                ...
                logger.error(
                    f"Error loading pros/cons from CSV for {self.company_id}: {e}"
                )
                ...
                with self._get_db_conn() as conn:
                    ...
                    if not df_db.empty:
                        ...
                        if p_str and pd.notna(p_str) and str(p_str).lower() != "nan":
                            pros.extend(
                                [p.strip() for p in str(p_str).split(";") if p.strip()]
                            )
                        if c_str and pd.notna(c_str) and str(c_str).lower() != "nan":
                            cons.extend(
                                [c.strip() for c in str(c_str).split(";") if c.strip()]
                            )
            except Exception as e:
                logger.error(
                    f"Error loading pros/cons DB fallback for {self.company_id}: {e}"
                )
            ...
            try:
                ...
                if "company_id" in df.columns:
                    ...
                    if not m.empty:
                        ...
                        res["pattern"] = str(
                            row.get("capital_allocation_pattern", "N/A")
                        )
                        res["latest_year"] = str(row.get("latest_year", "N/A"))
            except Exception as e:
                logger.error(
                    f"Error loading capital allocation for {self.company_id}: {e}"
                )
        return res
    ...
    def has_sufficient_data(self) -> Tuple[bool, str]:
        ...
        if bs_years < 3:
            return (
                False,
                f"Fewer than 3 years of Balance Sheet data ({bs_years} years found)",
            )
        if cf_years < 3:
            return (
                False,
                f"Fewer than 3 years of Cash Flow data ({cf_years} years found)",
            )
    ...
    def _create_revenue_profit_chart(self) -> Optional[str]:
        ...
        ax.set_title(
            "10-Year Historical Revenue & Net Profit (₹ Cr)",
            fontsize=9,
            fontweight="bold",
            pad=6,
            color="#1E293B",
    ...
    def _create_roe_roce_chart(self) -> Optional[str]:
        ...
        ax.set_title(
            "Historical Profitability Trends: ROE & ROCE (%)",
            fontsize=9,
            fontweight="bold",
            pad=6,
            color="#1E293B",
    ...
    def _create_bs_composition_chart(self) -> Optional[str]:
        ...
        other_liab = pd.to_numeric(
            df.get("other_liabilities", 0), errors="coerce"
        ).fillna(0)
    ...
    def _create_cf_waterfall_chart(self) -> Optional[str]:
        ...
        period = (
            str(latest_cf.get("period", "Latest")).replace("Mar ", "").replace("FY", "")
        )

        cfo = pd.to_numeric(
            latest_cf.get("cash_from_operating_activity", 0), errors="coerce"
        )
        cfi = pd.to_numeric(
            latest_cf.get("cash_from_investing_activity", 0), errors="coerce"
        )
        cff = pd.to_numeric(
            latest_cf.get("cash_from_financing_activity", 0), errors="coerce"
        )
        ...
        ax.set_title(
            f"Cash Flow Breakdown for {period} (₹ Cr)",
            fontsize=9,
            fontweight="bold",
            pad=6,
            color="#1E293B",
    ...
    def generate(self, output_path: Optional[Path] = None) -> Path:
        ...
        rev_val = (
            f"₹{pd.to_numeric(latest_pl.get('sales', 0), errors='coerce'):,.0f} Cr"
            if latest_pl.get("sales") is not None
            else "N/A"
        )
        pat_val = (
            f"₹{pd.to_numeric(latest_pl.get('net_profit', 0), errors='coerce'):,.0f} Cr"
            if latest_pl.get("net_profit") is not None
            else "N/A"
        )
        roe_val = (
            f"{pd.to_numeric(latest_kpi.get('roe', 0), errors='coerce'):.2f}%"
            if latest_kpi.get("roe") is not None
            else "N/A"
        )
        roce_val = (
            f"{pd.to_numeric(latest_kpi.get('roce', 0), errors='coerce'):.2f}%"
            if latest_kpi.get("roce") is not None
            else "N/A"
        )
        de_val = (
            f"{pd.to_numeric(latest_kpi.get('debt_to_equity', 0), errors='coerce'):.2f}x"
            if latest_kpi.get("debt_to_equity") is not None
            else "N/A"
        )
        ...
        kpi_tiles_data = [
            [
                [
                    Paragraph("REVENUE (SALES)", kpi_title_style),
                    Paragraph(rev_val, kpi_val_style),
                ],
                [
                    Paragraph("NET PROFIT (PAT)", kpi_title_style),
                    Paragraph(pat_val, kpi_val_style),
                ],
                [
                    Paragraph("RETURN ON EQUITY", kpi_title_style),
                    Paragraph(roe_val, kpi_val_style),
                ],
            ],
            [
                [
                    Paragraph("ROCE", kpi_title_style),
                    Paragraph(roce_val, kpi_val_style),
                ],
                [
                    Paragraph("DEBT-TO-EQUITY", kpi_title_style),
                    Paragraph(de_val, kpi_val_style),
                ],
                [
                    Paragraph("HEALTH SCORE", kpi_title_style),
                    Paragraph(health_val, kpi_val_style),
                ],
            ],
        ]
        ...
        elements.append(
            Paragraph("10-Year Financial Performance Trend", section_heading)
        )
        elements.append(
            HRFlowable(width="100%", thickness=1, color=PRIMARY_NAVY, spaceAfter=4)
        )

        rev_prof_chart = self._create_revenue_profit_chart()
        if rev_prof_chart and os.path.exists(rev_prof_chart):
            elements.append(
                Image(rev_prof_chart, width=printable_width, height=2.2 * inch)
            )
        else:
            elements.append(
                Paragraph(
                    "<i>Historical Revenue & Profit chart data unavailable.</i>",
                    body_style,
                )
            )
        ...
        elements.append(
            Paragraph(
                "Profitability & Efficiency Trends (ROE vs ROCE)", section_heading
            )
        )
        elements.append(
            HRFlowable(width="100%", thickness=1, color=PRIMARY_NAVY, spaceAfter=4)
        ...
        if roe_roce_chart and os.path.exists(roe_roce_chart):
            elements.append(
                Image(roe_roce_chart, width=printable_width, height=2.1 * inch)
            )
        else:
            elements.append(
                Paragraph(
                    "<i>ROE & ROCE historical trend chart data unavailable.</i>",
                    body_style,
                )
            )
        ...
        p2_table = Table(
            [[Paragraph(p2_header_text, title_style)]], colWidths=[printable_width]
        )
        p2_table.setStyle(
            TableStyle(
                [
        ...
        elements.append(
            Paragraph("Balance Sheet Capital Structure Composition", section_heading)
        )
        elements.append(
            HRFlowable(width="100%", thickness=1, color=PRIMARY_NAVY, spaceAfter=4)
        )
        ...
        if bs_chart and os.path.exists(bs_chart):
            ...
            elements.append(
                Paragraph(
                    "<i>Balance Sheet composition chart data unavailable.</i>",
                    body_style,
                )
            )
        ...
        elements.append(
            Paragraph("Latest Cash Flow Dynamics Waterfall", section_heading)
        )
        elements.append(
            HRFlowable(width="100%", thickness=1, color=PRIMARY_NAVY, spaceAfter=4)
        )
        ...
        if cf_chart and os.path.exists(cf_chart):
            ...
            elements.append(
                Paragraph(
                    "<i>Cash Flow waterfall chart data unavailable.</i>", body_style
                )
            )
        ...
        elements.append(
            Paragraph("NLP Automated Pros & Cons Intelligence", section_heading)
        )
        elements.append(
            HRFlowable(width="100%", thickness=1, color=PRIMARY_NAVY, spaceAfter=4)
        )
        ...
        pros_html = (
            "<br/>".join([f"• {p}" for p in pros_list[:3]])
            if pros_list
            else "• No significant positive signals detected."
        )
        cons_html = (
            "<br/>".join([f"• {c}" for c in cons_list[:3]])
            if cons_list
            else "• No major risk signals detected."
        )

        pro_header = Paragraph(
            "<b>Positive Signals (Pros)</b>",
            ParagraphStyle(
                "ProHead",
                parent=styles["Normal"],
                fontName="Helvetica-Bold",
        ...
        con_header = Paragraph(
            "<b>Risk Signals & Concerns (Cons)</b>",
            ParagraphStyle(
                "ConHead",
                parent=styles["Normal"],
                fontName="Helvetica-Bold",
...
def generate_company_tearsheet(
    company_id: str, output_path: Optional[Path] = None, db_path: Optional[Path] = None
) -> Path:
    """
...
def generate_batch_tearsheets(db_path: Optional[Path] = None) -> Dict[str, Any]:
    ...
    logger.info(
        f"Starting batch tearsheet generation for {total_companies} companies..."
    )
    ...
    for idx, row in companies_df.iterrows():
        ...
        try:
            ...
            if file_size_kb < 30.0:
                logger.warning(
                    f"Generated PDF for {cid} is under 30 KB ({file_size_kb:.1f} KB)"
                )
            ...
            failure_records.append(
                {"company_id": cid, "error": str(e), "stage": "tearsheet_generation"}
            )

    # Export logs
    skipped_df = (
        pd.DataFrame(skipped_records)
        if skipped_records
        else pd.DataFrame(columns=["company_id", "reason"])
    )
    ...
    failures_df = (
        ...
        if failure_records
        else pd.DataFrame(columns=["company_id", "error", "stage"])
    )
