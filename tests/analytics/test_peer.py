...
class TestValidationError:
    ...
    def test_validation_error_creation(self):
        ...
        error = ValidationError(
            "TEST001", "Energy", "MISSING_METRIC", "Metric not found"
        )
...
class TestLoadPeerGroups:
    ...
    def test_load_from_database(self, temp_db, sample_peer_groups):
        ...
        with patch("src.analytics.peer.get_connection") as mock_get_conn:
            ...
            # Insert sample data
            sample_peer_groups.to_sql(
                "peer_groups", mock_conn, if_exists="replace", index=False
            )
            mock_conn.commit()
    ...
    def test_load_from_excel_file_not_found(self):
        ...
        with patch("src.analytics.peer.RAW_DATA_DIR") as mock_raw_dir:
            mock_raw_dir.__truediv__ = MagicMock(
                return_value=Path("/nonexistent/peer_groups.xlsx")
            )
            mock_path = MagicMock()
...
class TestAssignPeerGroups:
    ...
    def test_assign_peer_groups_custom_column(self):
        ...
        df = pd.DataFrame({"comp_id": ["A", "B", "C"], "value": [1, 2, 3]})
...
class TestCalculateMetricPercentiles:
    ...
    def test_calculate_metric_with_groups(self, sample_financial_data):
        ...
        reliance_roe_pct = result[result["company_id"] == "RELIANCE"][
            "roe_percentile"
        ].iloc[0]
...
class TestCalculateAllPercentiles:
    ...
    def test_calculate_custom_metrics(self, sample_financial_data):
        ...
        result = calculate_all_percentiles(
            sample_financial_data, metrics=custom_metrics
        )
...
class TestDebtToEquityInversion:
    ...
    def test_debt_to_equity_inversion(self):
        ...
        lowest_debt_pct = result[result["company_id"] == "A"][
            "debt_to_equity_percentile"
        ].iloc[0]
        highest_debt_pct = result[result["company_id"] == "E"][
            "debt_to_equity_percentile"
        ].iloc[0]
...
class TestSavePeerPercentiles:
    ...
    def test_skip_companies_without_peer_group(
        self, temp_db, sample_financial_data_with_missing
    ):
...
class TestExportPercentiles:
    ...
    def test_export_with_period_filter(self, sample_financial_data, tmp_path):
        ...
        df = pd.concat(
            [sample_financial_data, sample_financial_data.copy()], ignore_index=True
        )
...
class TestPeerPercentileEngine:
    ...
    def test_pipeline_with_missing_peer_groups(
        self, sample_financial_data_with_missing
    ):
...
class TestEdgeCases:
    ...
    def test_missing_metrics_columns(self):
        ...
        df = pd.DataFrame({"company_id": ["A", "B"], "peer_group_name": ["G1", "G1"]})
...
class TestPerformance:
    ...
    def test_large_dataset_processing(self, large_dataset):
        ...
        assert (
            elapsed_time < 30.0
        ), f"Processing took {elapsed_time:.2f}s, expected < 30s"
    ...
    def test_percentile_calculation_performance(self, large_dataset):
        ...
        assert (
            elapsed_time < 10.0
        ), f"Calculation took {elapsed_time:.2f}s, expected < 10s"
    ...
    def test_batch_insert_performance(self, temp_db, large_dataset):
        ...
        with patch("src.analytics.peer.get_connection") as mock_get_conn:
            ...
            assert (
                elapsed_time < 20.0
            ), f"Insert took {elapsed_time:.2f}s, expected < 20s"
...
class TestIntegration:
    ...
    def test_debt_to_equity_inversion_integration(self, sample_financial_data):
        ...
        df = calculate_all_percentiles(
            sample_financial_data, metrics=["debt_to_equity"]
        )
